...
import type { SideNames } from '@/lib/types';

interface TopicInputProps {
  onSubmit: (topic: string, apiKey: string, sideNames: SideNames) => void;
}
...
export default function TopicInput({ onSubmit }: TopicInputProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (topic.trim() && apiKey.trim() && affirmativeName.trim() && negativeName.trim()) {
      onSubmit(topic.trim(), apiKey.trim(), {
        affirmative: affirmativeName.trim(),
        negative: negativeName.trim(),
      });
    }
  };

  const canSubmit = topic.trim() && apiKey.trim() && affirmativeName.trim() && negativeName.trim();
  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-4">
      ...
      {/* Topic + Side Names */}
      <div className="bg-white rounded-2xl shadow-sm border border-zinc-200 p-6">
        <h2 className="text-lg font-bold text-zinc-800 mb-1">ディベートの設定</h2>
        <p className="text-xs text-zinc-500 mb-4">
          論題と対戦する2つの立場の名前を入力してください
        </p>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-zinc-700 mb-1">論題</label>
            <textarea
              value={topic}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-blue-700 mb-1">立場 A</label>
              <input
                type="text"
                value={affirmativeName}
                onChange={e => setAffirmativeName(e.target.value)}
                placeholder="例: 賛成派"
                className="w-full px-4 py-3 border border-blue-200 rounded-xl text-zinc-800 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-blue-50/50"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-rose-700 mb-1">立場 B</label>
              <input
                type="text"
                value={negativeName}
                onChange={e => setNegativeName(e.target.value)}
                placeholder="例: 反対派"
                className="w-full px-4 py-3 border border-rose-200 rounded-xl text-zinc-800 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-transparent bg-rose-50/50"
              />
            </div>
          </div>
